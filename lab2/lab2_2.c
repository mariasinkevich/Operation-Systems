...
void *proc1(void *arg) {
    ...
    while (a1->flag == 0){
        int tryl = 0;
        while(tryl == 0){
            int mutexres1 = pthread_mutex_trylock(&mutex);
            if (mutexres1 == 0) {
                printf("\npthread_mutex_trylock 1 has completed correctly\n");
                tryl = 1; } }
        for (int i = 0; i < 10; i++){
            putchar(a1->sym);
            fflush(stdout);
...
void *proc2(void *arg) {
    ...
    while (a2->flag == 0){
        int tryl = 0;
        while(tryl == 0){
            int mutexres2 = pthread_mutex_trylock(&mutex);
            if (mutexres2 == 0) {
                printf("\npthread_mutex_trylock 2 has completed correctly\n");
                tryl = 1; } }
        for (int i = 0; i < 10; i++){
            putchar(a2->sym);
            fflush(stdout);
